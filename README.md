...
When Loco asks where to save the translation, choose the **System** location (`wp-content/languages/plugins/`). WordPress loads that folder automatically, and it survives plugin updates. The plugin's own `languages/` folder holds only the POT template: translations saved there are not loaded, because WordPress only scans the system folder unless a plugin registers a custom path, and this plugin follows HivePress core and all of its official extensions in not doing so.
